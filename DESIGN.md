...
2. **风控护栏（结构内生 · 必然属性）** —— 在每次动作前做刚性锚点校验 / 破窗止损 / 内 H 边界 / 第一 Bug 停机，不依赖模型自觉。这是**唯稳律结构的内生必然属性**（由推演公式 S+1 / D-1 双路导出，守真需以断保续），**DSH 适配层仅为其提供钩子载体，非其来源**；唯稳律原始定位即含此风控。

> 注：仓库 README 第一行主标语为"通用因果引擎（白箱呈现）"——与本文后续章节（映射表 / 6.1 反推）保持一致。风控护栏是唯稳律结构内生必然属性（由推演公式导出），DSH 适配层仅为其提供钩子载体，不可与本体姿态混淆。

---

## 0.1 层级划分：导图（心法）与 DSH（降维层）

唯稳律存在两层形态，二者是**源与投影**关系，不可混为一谈：

- **导图（心法）= 基础版（冻结）**：`M = (R×S)/(D×H)`，R/S/D/H 全部由调用方注入，全文件零领域实例。它是"因果律运行结构"的白箱呈现本体——一个数都没预设，故才有资格称"通用型因果"。
- **DSH（武功秘籍）= 本仓（活系统）**：导图的**降维投影**。工程层不降维转不起来，故把连续量离散为三态、预填判据（当前为 IT 实例）、加钩子载体。降维是必要代价，非框架缺口。

**降维的代价（作者 2026-09-02 收口）**：唯稳律是给**数字世界**用的——精准明确，才能执行。其容错率必然比真实世界的因果律**低**，这正是降维后的代价。导图层一个招式都没有，正因为它什么都是；DSH 层预填的判据只是当前的一个实例。

**铁律不随降维降级**：降维可丢表达力（连续量→三态、注入→预填判据），**不可丢缺省方向**（判不出来→review，不猜）。凡判"某处不通用"，先分清是降维代价还是框架缺口，禁止混为一谈。

**用户认知差异由用户自决**：层级如何被理解，是读者的自由；框架只提供精准明确的结构，不替读者解释。
