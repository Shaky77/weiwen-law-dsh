...
把 `weiwen-law.patch.yml` 作为 overlay 接入你的 DSH profile（具体路径以你的 DSH 版本为准，详见 [`DESIGN.md`](./DESIGN.md) 的挂载章节）。接入后，运行在该 profile 的 Agent 自动获得 5 个白箱工具。

> 注：原生挂载的精确 profile 路径随 DSH 版本变化；本仓库已通过实跑验证插件可被 DSH 加载、5 工具全部注册。如官方 API 有变更，以官方 docs 当前版本为准核对。

## 模型怎么调用（给 AI 工程师）

> **白话版**：插件向 DSH 注册 5 个白箱工具，模型像调普通函数一样调用它们来**自查边界**；同时挂了 3 道钩子做**硬性拦截**。
